...
A research lab runs a neural network with $n$ layers. Layer $i$ currently holds $\mathrm{layer}[i]$ neurons. Neurons can only be **added**, never removed. In each generation the lab may adjust **at most one** layer:

- On **odd** generations ($1, 3, 5, \ldots$): add **exactly 1** neuron to one layer, or do nothing.
- On **even** generations ($2, 4, 6, \ldots$): add **exactly 2** neurons to one layer, or do nothing.
...
- Each of the next $n$ lines: one integer $\mathrm{layer}[i]$.
